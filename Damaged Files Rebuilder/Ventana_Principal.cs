...
using Damaged_Files_Rebuilder.Properties;

namespace Damaged_Files_Rebuilder
{
    public partial class Ventana_Principal : Form
    {
        ...
        internal static readonly string Ruta_Reconstrucciones = Application.StartupPath + "\\Rebuilds";

        private void Ventana_Principal_Load(object sender, EventArgs e)
        {
            try
            {
                Program.Crear_Carpetas(Ruta_Reconstrucciones);
                if (Program.Icono_Jupisoft == null) Program.Icono_Jupisoft = this.Icon.Clone() as Icon;
                this.Text = Texto_Título + " - [Warning: never recombine different file types as one]";
                ...
                for (int Índice_Panel = 0, Índice_Bloque = 0; Índice_Panel < 4; Índice_Panel++)
                {
                    for (int Índice_Y = 0; Índice_Y < 5; Índice_Y++)
                    {
                        for (int Índice_X = 0; Índice_X < 5; Índice_X++, Índice_Bloque++)
                        {
                            ...
                            Combo.Items.Add("1º");
                            Combo.Items.Add("2º");
                            Combo.KeyDown += new KeyEventHandler(Ventana_Principal_KeyDown);
        ...
        private void Menú_Contextual_Visor_Ayuda_Click(object sender, EventArgs e)
        {
            try
            {
                MessageBox.Show(this, "This tool is meant to be used with multiple damaged copies of a single file each time (like one downloaded multiple times, and all with different errors due to a bad internet connection). It's highly recommended to use odd number of files, starting at 3 (only 2 damaged copies won't be enough since it needs 3 to make 1 perfect).", Program.Texto_Título_Versión, MessageBoxButtons.OK, MessageBoxIcon.Question);
            }
            catch (Exception Excepción) { Depurador.Escribir_Excepción(Excepción != null ? Excepción.ToString() : null); Variable_Excepción_Total++; Variable_Excepción = true; }
        }
            ...
            try
            {
                ...
                if (Lista_Rutas != null && Lista_Rutas.Count > 0)
                {
                    ...
                    if (Lista_Lectores.Count > 0)
                    {
                        ...
                        string Nombre = Program.Obtener_Nombre_Temporal();
                        string Ruta_Salida = Ruta_Reconstrucciones + "\\" + Nombre;
                        Program.Crear_Carpetas(Ruta_Salida);
                        Ruta_Salida += "\\" + Nombre + Matriz_Extensiones[0].Key;
                        Lector = new FileStream(Ruta_Salida, FileMode.CreateNew, FileAccess.Write, FileShare.ReadWrite);
